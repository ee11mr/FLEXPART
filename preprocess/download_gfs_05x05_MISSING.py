    ...
    if int(YEAR)==2017:
        ...
        for t in ["0000", "0600", "1200", "1800"]:
            for t2 in ["000"]:
                #for su in [".grb",".grb2"]:
                for su in [".grb2"]:
                    filelist.append("gfsanl_4_"+d.strftime('%Y%m%d')+"_"+t+"_"+t2+su)
        return filelist
    else:
        filelist = list()
        for t in ["0000", "0600", "1200", "1800"]:
            for t2 in ["000"]:
                filelist.append("gfsanl_4_"+d.strftime('%Y%m%d')+"_"+t+"_"+t2+suffix)
        return filelist
...
missing=[]
missing_file = open('/users/mjr583/scratch/flexpart/preprocess/gfs_data/code/missingfiles_05x05.txt', 'w+')
print('open missing and begin' )
# connect to ftp
...
for d in date_list:
    ...
    try:
        ...
        for f in file_list:
            if f in file_list_ftp:
                ...
                while os.stat(destination+f).st_size==0 and count < 50:
                    ...
                    download_file(path,f,destination)
                    count+=1
                cp_command= "cp "+destination+f+" /users/mjr583/scratch/flexpart/preprocess/flex_extract/05x05/GF"+f[11:13]+f[13:15]+f[15:17]+f[18:20]
                os.system(cp_command)
                last_file=destination+f
            else:
                missing_file.write("".join(f)+ "\n")
                print("  ", f, "not found: copying previous wind file")
                cp_command= "cp "+last_file+" /users/mjr583/scratch/flexpart/preprocess/flex_extract/05x05/GF"+f[11:13]+f[13:15]+f[15:17]+f[18:20]
                os.system(cp_command)
        ftp.cwd("../../../../")
    except:
        file_list=create_filenames(d)
        for f in file_list:
            ...
            cp_command= "cp "+last_file+" /users/mjr583/scratch/flexpart/preprocess/flex_extract/05x05/GF"+f[11:13]+f[13:15]+f[15:17]+f[18:20]
            os.system(cp_command)
open("LOGS/missing_05X05_SUCCESS",'a').close()
#ftp.quit()
ftp.close()

#missing_file = open('/users/mjr583/scratch/flexpart/preprocess/gfs_data/code/missing_05x05.txt', 'w+')
#if not missing:
