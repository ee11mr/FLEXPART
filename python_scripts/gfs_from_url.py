...
for date in datelist:
    ...
    for h,f in enumerate(filelist):
        if date > datetime.datetime(2020,5,17):
            ...
            file_url=url+date.strftime('%Y%m')+'/'+date.strftime('%Y%m%d')+'/'+f
        #print('/users/mjr583/scratch/flexpart/preprocess/flex_extract/05x05/GF'+date.strftime('%y%m%d')+hour[h])
        if not os.path.exists('/users/mjr583/scratch/flexpart/preprocess/flex_extract/05x05/GF'+date.strftime('%y%m%d')+hour[h]):
            try:
