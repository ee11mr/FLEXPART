...
for date in array_dates:
    ...
    ncdf_file.description = 'Trajectories calculated using FLEXPART v10.4, driven by GFS meteorological reanalyses. Number of particles = '+str(nparticles)+'. Point of release = Cape Verde Atmospheric Observatory (16.51 N, -24.52 W)'
    ncdf_file.references = 'https://www.geosci-model-dev.net/12/4955/2019/gmd-12-4955-2019-discussion.html'

    # Dimensions
    ncdf_file.createDimension('seconds', len(all_lat))
    ncdf_file.createDimension('datetime',len(all_lat))
    ncdf_file.createDimension('particle',nparticles)
    step=864000/len(all_lat)

    # Variables
    time = ncdf_file.createVariable('seconds_since_release',np.int32, ('seconds'))
    times = np.arange(step,864000+step,step)
