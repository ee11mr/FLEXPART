...
def gen_file_list(year, month=False, end=False,res='05x05',add=''):
    print('Get file list')
    file_list=[]
    year=str(year)
    if month:
        month=str(datetime(int(year), int(month), 1).strftime('%m'))   
        traj='/users/mjr583/scratch/flexpart/cvao/'+RES+'/'+year+'/netcdfs/'+year+month+'*.nc'
    if not month:
        traj='/users/mjr583/scratch/flexpart/cvao/'+RES+'/'+year+'/netcdfs/'+year+'*.nc'
...
outpath='/users/mjr583/scratch/flexpart/cvao/05x05/airmass_files/'
...
df.index = dt
df.index=pd.to_datetime(df.index, format='%d/%m/%Y %H:%M').strftime('%Y-%m-%d %H:%M')
...
df.to_csv(outpath+'CVAO_sector_%_boxes_1.csv',sep=",", mode='a', header=False)
